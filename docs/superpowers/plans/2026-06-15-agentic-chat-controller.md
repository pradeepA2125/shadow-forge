...
**NEXT:** **G** (`controller_factory.select_chat_handler` + flag wiring in route — also clears review #9) → **H** (H1 invariant guards spec §9 — INCLUDE review #5 `shadow==real` cross-file-reject test + H2 full suite/lint/type; **review at H2**) → **I** (frontend; **needs `npm install` at repo root first**) → **J** (live smoke — interactive dev-host, needs the human) → **K** (delete legacy explore→classify→route).
