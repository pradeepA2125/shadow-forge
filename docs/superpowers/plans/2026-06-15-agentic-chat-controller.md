...
**NEXT:** ⏳ **H2 CODE REVIEW (user asked)** — scope: post-F-review fixes (`a94cc63`) + G factory + H1 invariants. Then → **I** (frontend; **needs `npm install` at repo root first** — `controller.ts:85`+`webview-ui/src/types.ts:56` `LiveGateView.kind` += mode/edit → `LiveSlot.GateDispatch` ModeGate/EditGate; decision posts in `chat-panel.ts::registerHandlers`; `controller.ts:1456` `renderLiveGate` relax off `activeTaskId`) → **J** (live smoke — interactive dev-host, needs the human; flip `AI_EDITOR_CHAT_CONTROLLER=1`) → **K** (delete legacy explore→classify→route at `=0` retirement).
